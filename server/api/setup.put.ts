...
import type { ResponseData as ImageResponseData } from './image.put';
...
export interface ResponseData {
    error: { status: 1 | 2 | 3 | 4 | 5 | 403; message: string } | null;
    data: { id: number } | null;
}
...
export default defineEventHandler(async (event) => {
    ...
    if (body.image) {
        const response = await event.$fetch<ImageResponseData>('/api/image', {
            method: 'PUT',
            body: {
                image: body.image,
                resolution: 1920,
