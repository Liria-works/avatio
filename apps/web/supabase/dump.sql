...
CREATE
OR REPLACE FUNCTION "public"."gen_random_key"("bytea_length" integer) RETURNS "text" LANGUAGE "sql" AS $ _ $
SELECT
    string_agg(
        lpad(
            to_hex(width_bucket(random(), 0, 1, 256) - 1),
            2,
            '0'
        ),
        ''
...
CREATE
OR REPLACE FUNCTION "public"."get_public_id"("user_id" "uuid") RETURNS "text" LANGUAGE "plpgsql" AS $ $ DECLARE result text;

BEGIN
SELECT
    public_id INTO result
FROM
    public.users
WHERE
    id = user_id;
...
CREATE
OR REPLACE FUNCTION "public"."search_tags"("keywords" "text") RETURNS TABLE("tag" "text", "score" double precision) LANGUAGE "plpgsql" AS $ $ BEGIN RETURN QUERY
SELECT
    DISTINCT t.tag,
...
CREATE INDEX "pgroonga_search_tags_index" ON "public"."setup_tags" USING "pgroonga" ("tag") WITH (
    "normalizers" = 'NormalizerNFKC100("unify_to_romaji", true, "unify_hyphen_and_prolonged_sound_mark", true)',
    "tokenizer" = 'TokenBigramSplitSymbolAlphaDigit',
    "token_filters" = 'TokenFilterNFKC100("unify_to_romaji", true, "unify_kana", true, "unify_hyphen_and_prolonged_sound_mark", true)'
);

CREATE INDEX "pgroonga_setup_search_index_description" ON "public"."setups" USING "pgroonga" ("description") WITH (
    "normalizers" = 'NormalizerNFKC100("unify_to_romaji", true, "unify_hyphen_and_prolonged_sound_mark", true)',
    "tokenizer" = 'TokenBigramSplitSymbolAlphaDigit',
    "token_filters" = 'TokenFilterNFKC100("unify_to_romaji", true, "unify_kana", true, "unify_hyphen_and_prolonged_sound_mark", true)'
);
